...
RPS Game is a decentralized rock-paper-scissors game on Polkadot — on-chain leaderboard and game history, with solo play and Statement Store multiplayer. It runs embedded in a Polkadot **host** — Polkadot Mobile, Desktop, or Web — and signing is always approved on Polkadot Mobile (see [Hosts & signing](#hosts--signing)). Built with React + Vite + TypeScript on the Host API.
